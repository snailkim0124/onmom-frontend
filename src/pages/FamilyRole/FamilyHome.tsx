...
const FamilyHome = ({ setActiveTab }: FamilyHomeProps) => {
  ...
  const [familyInfo, setFamilyInfo] = useState({
    ...
    wifeMessage: "오늘 퇴근길에 🍓 딸기 사다 주면 좋겠어 여보! ❤️",
    wifeMessageTime: "1시간 전",
    nextHospitalVisit: "7월 15일 (수) 오전 10시",
    babyTip: "아빠, 엄마가 요즘 다리가 많이 붓는대요. 오늘 퇴근하고 엄마 다리 꼭 주물러 주세요!",
  });

  // 🚀 2. 화면이 마운트될 때 로그인(KakaoCallback) 단계에서 저장된 실데이터 동기화
  useEffect(() => {
    ...
    setFamilyInfo((prev) => ({
      ...prev,
      // 실제 로그인 정보가 세션에 탑재되어 있다면 우선 교체, 없으면 기본값 유지
      husbandNickname: savedNickname || prev.husbandNickname,
      babyName: savedBabyName || prev.babyName,
      weeks: savedWeek ? Number(savedWeek) : prev.weeks,
    }));
  }, []);

  // 🚀 아직 구현되지 않은 서브 기능 카드 알림 핸들러
  ...
  return (
      <div className="flex min-h-screen flex-col bg-appBg p-5 pb-24 overflow-y-auto gap-6 animate-fade-in">
        ...
        {/* 2. 남편 전용 AI 코칭 리포트 구역 */}
        <div className="w-full rounded-2xl bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-200/50 via-[#F8F9FF] to-[#E5E8FF] p-6 shadow-md border border-indigo-100/60 flex flex-col gap-4">
          <div>
          ...
          <button
              onClick={() => setActiveTab("chat")} // 🚀 App.tsx의 activeTab 'chat' 분기와 통일하여 FamilyChat(FamilyReport) 오픈
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm font-medium text-gray-700 mt-0.5">
                "{familyInfo.wifeMessage}"
              </p>
            </div>
            <span className="text-xs text-gray-400">{familyInfo.wifeMessageTime}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-xl bg-white px-5 py-4 shadow-sm border border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400 font-bold">
              ...
              {familyInfo.nextHospitalVisit}
            </span>
            </div>
          </div>
          <button
              onClick={() => handleFeatureClick("동행 체크 변경")}
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {/* 오늘의 미션 */}
            <div
                onClick={() => handleFeatureClick("오늘의 미션")}
                className="cursor-pointer rounded-2xl bg-white p-4 shadow-sm border border-gray-100/50 active:scale-[0.98] transition flex flex-col gap-2"
            >
              <span className="text-2xl">✨</span>
            ...
            {/* 주차별 정보 */}
            <div
                onClick={() => handleFeatureClick("주차별 정보")}
                className="cursor-pointer rounded-2xl bg-white p-4 shadow-sm border border-gray-100/50 active:scale-[0.98] transition flex flex-col gap-2"
            >
              <span className="text-2xl">💡</span>
            ...
            {/* AI 리포트 (App.tsx activeTab 상태에 맞춘 맵핑) */}
            <div
                onClick={() => setActiveTab("chat")} // 🚀 대화 탭으로 라우팅 단일화
                className="cursor-pointer rounded-2xl bg-white p-4 shadow-sm border border-gray-100/50 active:scale-[0.98] transition flex flex-col gap-2"
            >
              <span className="text-2xl">📝</span>
