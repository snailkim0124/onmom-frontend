...
const FamilyReport = () => {
  ...
  const [reportData, setReportData] = useState<FamilyMessageData>({
    ...
    aiCommentary:
      "오늘 아내분은 체중 증가와 혈액량 변화로 인해 다리가 많이 붓고 피로를 느끼고 있어요. 평소보다 조금 더 세심한 배려가 필요한 날입니다. 무거운 물건은 대신 들어주시고, 잠들기 전 따뜻한 물로 족욕을 준비해 주시면 아내의 컨디션 회복에 아주 큰 도움이 될 거예요!",
  ...
  useEffect(() => {
    const fetchFamilyReport = async () => {
      ...
      try {
        ...
        console.log("백엔드 수신 메시지 API 호출: GET /api/v1/family-messages");
          ...
          <div className="flex items-center gap-3">
            <CheckCircleFilled
              className={`text-xl transition-colors ${
                reportData.isActionCompleted
                  ? "text-green-500"
                  : "text-gray-300"
              }`}
            />
            <span
              className={`font-gowun text-sm font-bold transition-all ${
                reportData.isActionCompleted
